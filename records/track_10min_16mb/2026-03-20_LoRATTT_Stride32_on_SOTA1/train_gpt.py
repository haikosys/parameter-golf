...
class BatchedLinearLoRA(nn.Module):
    ...
    def reset(self) -> None:
        with torch.no_grad():
            self.A.uniform_(-1.0 / math.sqrt(self.in_f), 1.0 / math.sqrt(self.in_f))
            self.B.zero_()
...
def _find_docs(all_tokens: Tensor) -> list[tuple[int, int]]:
    bos = (all_tokens == BOS_ID).nonzero(as_tuple=True)[0].numpy()
    docs = []
    for i in range(len(bos)):
        start = int(bos[i])
        end = (int(bos[i + 1]) + 1) if i + 1 < len(bos) else all_tokens.numel()
        if end - start >= 2:
            docs.append((start, end - start))
    return docs

def _chunk_window(ci, pred_len, num_chunks, chunk_size, eval_seq_len):
    cs = ci * chunk_size
...
def eval_val_ttt_lora(
    ...
    chunk_sz, eval_sl, bsz = 256, 1024, 64
    lora_rank, lora_lr = 8, 0.01
    my_docs.sort(key=lambda d: (d[1] - 2) // chunk_sz)
    base_model.eval()
    for p in base_model.parameters():
        p.requires_grad_(False)
    lora = BatchedTTTLoRA(bsz, base_model, lora_rank).to(device)
    opt = torch.optim.Adam(lora.parameters(), lr=lora_lr, betas=(args.beta1, args.beta2), eps=1e-10)
    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    byte_sum = torch.zeros((), device=device, dtype=torch.float64)
    tok_count = torch.zeros((), device=device, dtype=torch.float64)
    for bi in range(0, len(my_docs), bsz):
        batch = my_docs[bi:bi + bsz]
        bs = len(batch)
        if bs == bsz:
            lora.reset()
            ...
            lora = BatchedTTTLoRA(bs, base_model, lora_rank).to(device)
            opt = torch.optim.Adam(lora.parameters(), lr=lora_lr, betas=(args.beta1, args.beta2), eps=1e-10)
        pls = [dl - 1 for _, dl in batch]
        ncs = [(pl + chunk_sz - 1) // chunk_sz for pl in pls]
        for ci in range(max(ncs)):
            cstats = _chunk_window(ci, (ci + 1) * chunk_sz, ci + 1, chunk_sz, eval_sl)
            ctx_sz, co = cstats[1], cstats[2]
            active = [ci < nc for nc in ncs]
            needs_train = any(ci < nc - 1 for nc in ncs)
            x = torch.zeros(bs, ctx_sz, dtype=torch.int64, device=device)
            y = torch.zeros(bs, ctx_sz, dtype=torch.int64, device=device)
            dinfo = []
            for b in range(bs):
                if not active[b]:
                    dinfo.append((0, 0)); continue
                ds, dl = batch[b]
                ws, wl, lco, lcl = _chunk_window(ci, pls[b], ncs[b], chunk_sz, eval_sl)
            ...
            with torch.no_grad():
                for b in range(bs):
                    ...
                    lco, lcl = dinfo[b]
                    lbl = ptl[b, lco:lco + lcl].to(torch.float64)
                    tgt = y[b, lco:lco + lcl]; prev = x[b, lco:lco + lcl]
                    tb = base_bytes_lut[tgt].to(torch.float64)
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    loss_sum += lbl.sum(); byte_sum += tb.sum(); tok_count += lcl
            if needs_train:
                mask = torch.tensor([float(ci < ncs[b] - 1) for b in range(bs)], device=device)
                per_doc = ptl[:, co:co + chunk_sz].mean(dim=-1)
    ...
    return vl, vl / math.log(2.0) * (tok_count.item() / byte_sum.item())
...
def main() -> None:
    ...
    # LoRA TTT evaluation (per-document adaptation)
    torch.cuda.synchronize()
    ...
    torch.cuda.synchronize()
    log0(f"final_ttt_lora val_loss:{ttt_loss:.4f} val_bpb:{ttt_bpb:.4f} eval_time:{1000*(time.perf_counter()-t_ttt):.0f}ms")
