"""
Jupiter submission — gap_closer council optimizations on Mars base.
NS3, DDP hardening, cosine warmdown + micro-restart, progressive seq_len,
per-channel QAT scale consensus, signSGD TTT.
"""
...
class Hyperparameters:
    ...
    prog_seq_switch = int(os.environ.get("PROG_SEQ_SWITCH", 0))
    prog_seq_short = int(os.environ.get("PROG_SEQ_SHORT", 1024))
    scale_consensus = bool(int(os.environ.get("SCALE_CONSENSUS", "1")))
    scale_consensus_lambda = float(os.environ.get("SCALE_CONSENSUS_LAMBDA", 0.1))
    ttt_sign_sgd = bool(int(os.environ.get("TTT_SIGN_SGD", "1")))
    ttt_stride_aware = bool(int(os.environ.get("TTT_STRIDE_AWARE", "1")))
...
def scale_consensus_loss(model, lambda_reg=0.1):
    """Regularize attention projection weight scales toward median for better int6 quantization.

    During QAT, per-row scales can diverge wildly across output channels.
    This loss nudges them toward their median, making the exported per-row
    int6 scales more uniform and reducing worst-case quantization error.
    Only targets CastedLinear layers inside attention sub-modules.
    """
    loss = 0.0
    count = 0
    for name, module in model.named_modules():
        if isinstance(module, CastedLinear) and ('attn' in name) and module.weight.ndim == 2:
            w = module.weight.float()
            scales = w.abs().amax(dim=1)
            target = scales.median()
            loss = loss + (scales - target).pow(2).mean()
            count += 1
    if count == 0:
        return torch.zeros((), device=next(model.parameters()).device)
    return lambda_reg * loss / count
...
def ttt_adapt(args, base_model, device, val_tokens, rank=0, world_size=1, log_fn=None):
    ...
    if args.ttt_sign_sgd:
        optimizer = SignSGD(ttt_params, lr=args.ttt_lr)
    else:
...
def scale_consensus_loss(model, lambda_reg=0.1):
    ...
    for name, module in model.named_modules():
        if isinstance(module, CastedLinear) and 'attn' in name and module.weight.ndim == 2:
            ...
            loss = loss + (scales - target).pow(2).mean()
            count += 1
    return lambda_reg * loss / max(count, 1)
...
def main() -> None:
    ...
    model: nn.Module = DDP(compiled_model, device_ids=[local_rank], broadcast_buffers=False, gradient_as_bucket_view=True, static_graph=True) if distributed else compiled_model
    ...
    cosine_warmdown = int(os.environ.get("COSINE_WARMDOWN", "1"))
    micro_restart = int(os.environ.get("MICRO_RESTART", "1"))
    micro_restart_lr = float(os.environ.get("MICRO_RESTART_LR", "0.25"))
    micro_restart_steps = int(os.environ.get("MICRO_RESTART_STEPS", "200"))
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        ...
        if max_wallclock_ms is None:
            ...
            progress = steps_into_warmdown / max(args.warmdown_iters, 1)
        else:
            ...
            steps_into_warmdown = int(progress * args.warmdown_iters)
        progress = min(max(progress, 0.0), 1.0)
        # Micro-restart: at 80% through warmdown, bump LR for micro_restart_steps
        if micro_restart and progress >= 0.8:
            restart_start = int(0.8 * args.warmdown_iters)
            restart_step = steps_into_warmdown - restart_start
            if 0 <= restart_step < micro_restart_steps:
                micro_progress = restart_step / max(micro_restart_steps, 1)
                return micro_restart_lr * 0.5 * (1.0 + math.cos(math.pi * micro_progress))
        if cosine_warmdown:
    ...
    while True:
        ...
        if args.prog_seq_switch > 0 and step == args.prog_seq_switch:
            log0(f"prog_seq:switch to seq_len={args.train_seq_len} at step:{step}")
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            cur_seq_len = args.prog_seq_short if (args.prog_seq_switch > 0 and step < args.prog_seq_switch) else args.train_seq_len
            x, y = train_loader.next_batch(args.train_batch_tokens, cur_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
            if args.scale_consensus and CastedLinear._qat_enabled:
                sc_loss = scale_consensus_loss(base_model, args.scale_consensus_lambda)
                loss = loss + sc_loss
