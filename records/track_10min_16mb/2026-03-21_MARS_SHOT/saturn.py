...
try:
    ...
    for _egg in _glob.glob("/usr/local/lib/python*/dist-packages/flash_attn_3*.egg"):
        if _egg not in sys.path:
            sys.path.insert(0, _egg)
    from flash_attn_interface import flash_attn_func as flash_attn_3_func
    _ATTN_BACKEND = "flash_attn_3"
except ImportError:
    try:
        from flash_attn import flash_attn_func as flash_attn_3_func
        _ATTN_BACKEND = "flash_attn_2"
    except ImportError:
        _ATTN_BACKEND = "sdpa_fallback"
...
print(f"[attn_backend] {_ATTN_BACKEND}")
...
class Hyperparameters:
    ...
    sdttt_epochs = int(os.environ.get("SDTTT_EPOCHS", 2))
    sdttt_lr = float(os.environ.get("SDTTT_LR", 0.001))
    sdttt_temperature = float(os.environ.get("SDTTT_TEMPERATURE", 2.0))
    sdttt_freeze_blocks = int(os.environ.get("SDTTT_FREEZE_BLOCKS", 4))

    # GPTQ-lite: search optimal clipping percentile per layer during quantization.
    gptq_enabled = bool(int(os.environ.get("GPTQ_ENABLED", "0")))
    gptq_calib_seqs = int(os.environ.get("GPTQ_CALIB_SEQS", 128))
...
class CausalSelfAttention(nn.Module):
    ...
    def _eodiff(self, y: Tensor) -> Tensor:
        """Even/odd head differential: zero-cost noise cancellation.
        Splits heads into even/odd pairs, subtracts to cancel shared noise,
        concatenates differential + sum heads to preserve output dim."""
        B, T, H, D = y.shape
        y_even = y[:, :, 0::2, :]  # heads 0,2,4,6 → (B, T, H/2, D)
        y_odd = y[:, :, 1::2, :]   # heads 1,3,5,7 → (B, T, H/2, D)
        lam = self.eodiff_lambda.to(dtype=y.dtype)
        diff = y_even - lam * y_odd    # noise-cancelled
        summ = y_even + y_odd          # preserved signal
        # Interleave back: diff heads in even positions, sum in odd
        out = torch.stack([diff, summ], dim=3)  # (B, T, H/2, 2, D)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Enable even/odd head differential (replaces XSA on those layers)
        if eodiff_last_n > 0:
            for i in range(max(0, num_layers - eodiff_last_n), num_layers):
                self.blocks[i].attn.use_eodiff = True
                self.blocks[i].attn.use_xsa = False  # eodiff replaces XSA
        self._init_weights()
...
def _block_hadamard(n: int) -> Tensor:
    if n > 0 and (n & (n - 1)) == 0:
        return _hadamard(n)
    block = 1
    ...
    return torch.block_diag(*([_hadamard(block)] * (n // block)))

_hadamard_cache: dict[int, Tensor] = {}
def _get_hadamard(n: int) -> Tensor:
    if n not in _hadamard_cache:
        _hadamard_cache[n] = _block_hadamard(n)
    return _hadamard_cache[n]

def optrot_rotate_sd(sd: dict[str, Tensor], int6_cats: set[str]) -> dict[str, Tensor]:
    rotated = {}
    for name, t in sd.items():
        cat = _classify_param(name)
        if cat in int6_cats and t.ndim == 2 and t.shape[1] >= 4:
            H = _get_hadamard(t.shape[1])
            rotated[name] = (t.double() @ H).to(t.dtype)
        else:
            rotated[name] = t
    return rotated

def optrot_unrotate_sd(sd: dict[str, Tensor], int6_cats: set[str]) -> dict[str, Tensor]:
    unrotated = {}
    for name, t in sd.items():
        cat = _classify_param(name)
        if cat in int6_cats and t.ndim == 2 and t.shape[1] >= 4:
            H = _get_hadamard(t.shape[1])
            unrotated[name] = (t.double() @ H).to(t.dtype)
        else:
            unrotated[name] = t
    return unrotated
...
def quantize_int6_per_row(t: Tensor, gptq: bool = False) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        if gptq:
            # GPTQ-lite: search for optimal clip ratio per row to minimize reconstruction error.
            # Instead of using raw amax, try clipping at different percentiles.
            best_q = None
            best_scale = None
            best_err = float("inf")
            for clip_pct in [1.0, 0.999, 0.995, 0.99, 0.98]:
                if clip_pct < 1.0:
                    row_max = torch.quantile(t32.abs(), clip_pct, dim=1)
                else:
                    row_max = t32.abs().amax(dim=1)
                scale = (row_max / 31.0).clamp_min(1.0 / 31.0).to(torch.float16)
                clipped = torch.clamp(t32, -row_max[:, None], row_max[:, None])
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], gptq: bool = False):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, gptq=gptq)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def sdttt_adapt(args, base_model, device, val_tokens, rank=0, world_size=1, log_fn=None):
    """Self-distillation TTT: adapt to val data using KL divergence from frozen teacher.
    Unlike hard-label TTT, this preserves XSA attention patterns because the KL loss
    keeps the student close to the teacher's output distribution."""
    seq_len = args.train_seq_len
    total_seqs = (val_tokens.numel() - 1) // seq_len
    T = args.sdttt_temperature

    # Freeze early blocks for stability
    frozen = set()
    if args.sdttt_freeze_blocks > 0:
        for i, block in enumerate(base_model.blocks):
            if i < args.sdttt_freeze_blocks:
                for p in block.parameters():
                    p.requires_grad_(False)
                    frozen.add(id(p))

    params = [p for p in base_model.parameters() if p.requires_grad]
    optimizer = torch.optim.SGD(params, lr=args.sdttt_lr, momentum=0.9)

    my_start = (total_seqs * rank) // world_size
    my_end = (total_seqs * (rank + 1)) // world_size
    batch_seqs = 32

    # Snapshot teacher logits (frozen copy)
    ...
    for epoch in range(args.sdttt_epochs):
        ...
        for bs in range(my_start, my_end, batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                # Single forward pass: get logits, compute both CE and KL from them
                student_logits = base_model.forward_logits(x)  # (B, T, V)
                logits_flat = student_logits.reshape(-1, student_logits.size(-1)).float()
                targets_flat = y.reshape(-1)
                student_loss = F.cross_entropy(logits_flat, targets_flat, reduction="mean")
                with torch.no_grad():
                    teacher_logits = teacher_model.forward_logits(x).reshape(-1, student_logits.size(-1)).float()
                student_log_probs = F.log_softmax(logits_flat / T, dim=-1)
                teacher_probs = F.softmax(teacher_logits / T, dim=-1)
                kl_loss = F.kl_div(student_log_probs, teacher_probs, reduction="batchmean") * (T * T)
                loss = 0.7 * kl_loss + 0.3 * student_loss
        ...
        if log_fn:
            log_fn(f"sdttt_epoch:{epoch+1}/{args.sdttt_epochs} loss:{epoch_loss.item()/max(epoch_tokens.item(),1):.4f} time:{time.perf_counter()-t0:.1f}s")
...
def main() -> None:
    ...
    # -----------------------------
    # SELF-DISTILLATION TTT (post-training, eval-time)
    # -----------------------------
    if args.sdttt_enabled:
        log0("sdttt:starting self-distillation TTT")
        sdttt_adapt(args, base_model, device, val_tokens, rank, world_size, log_fn=log0)
        # Diagnostic eval after SDTTT
        torch.cuda.synchronize()
        t_sdttt = time.perf_counter()
        sd_loss, sd_bpb = eval_val(
            args, compiled_model, rank, world_size, device, grad_accum_steps,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        )
        torch.cuda.synchronize()
        log0(f"DIAGNOSTIC post_sdttt val_loss:{sd_loss:.4f} val_bpb:{sd_bpb:.4f} eval_time:{1000.0*(time.perf_counter()-t_sdttt):.0f}ms")
    ...
    if args.optrot_enabled:
        log0("optrot:rotating weights (Hadamard right-multiply) before quantization")
        sd_cpu = optrot_rotate_sd(sd_cpu, {"mlp", "attn"})
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, gptq=args.gptq_enabled)
    if args.gptq_enabled:
        log0("gptq:enabled — per-layer optimal clip search")
    ...
    if args.optrot_enabled:
        log0("optrot:un-rotating dequantized weights")
        deq_state = optrot_unrotate_sd(deq_state, {"mlp", "attn"})
