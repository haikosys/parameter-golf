"""train_gpt.py — TITAN: Star-ReLU + 4KV + MLP1792 + Adaptive int7/6/5 + GPTQ per-row + Backout. NO TTT."""
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear_gate,bigram,skip_gates,backout_alpha",
    ).split(",")
    if pattern
)
...
def quantize_float_tensor(t: Tensor, nbits: int = 6, gptq_search: bool = False) -> tuple[Tensor, Tensor]:
    """Quantize to int{nbits} range, stored as int8. GPTQ per-row clip search if enabled."""
    qmin = -(1 << (nbits - 1))
    qmax = (1 << (nbits - 1)) - 1
    t32 = t.float()
    if t32.ndim == 2:
        if gptq_search:
            ...
            for pct in [0.999, 0.9995, 0.9999, 0.99999, 1.0]:
                ...
                sc = (ca / float(qmax)).clamp_min(1.0 / float(qmax)).to(torch.float16)
                clipped = torch.clamp(t32, -ca[:, None], ca[:, None])
                q = torch.clamp(torch.round(clipped / sc.float()[:, None]), qmin, qmax).to(torch.int8)
                recon = q.float() * sc.float()[:, None]
                mse = ((t32 - recon) ** 2).sum().item()
                if mse < best_mse:
                    best_mse, best_q, best_s = mse, q, sc
            return best_q.contiguous(), best_s.contiguous()
        clip_abs = torch.quantile(t32.abs(), INT6_CLIP_Q, dim=1) if t32.numel() else torch.empty((t32.shape[0],), dtype=torch.float32)
...
def _classify_sensitivity(name: str, layer_idx: int, num_layers: int) -> str:
    """Classify tensor sensitivity for adaptive bitwidth: 'high', 'medium', 'low'."""
    if 'tok_emb' in name or 'lm_head' in name:
        return 'high'
    if '.c_q.' in name or '.c_k.' in name:
        if layer_idx >= num_layers - 4:
            return 'high'
    if '.mlp.' in name and layer_idx < num_layers // 3:
        return 'low'
    return 'medium'
...
def quantize_state_dict_int6(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        if adaptive and t.ndim == 2 and li >= 0:
            ...
            nbits = {'high': 7, 'medium': 6, 'low': 5}[sens]
        else:
            nbits = 6
        q, s = quantize_float_tensor(t, nbits=nbits, gptq_search=gptq)
        if s.ndim > 0:
            qmeta[name] = {"scheme": "per_row", "axis": 0, "nbits": nbits}
        quantized[name] = q
        scales[name] = s
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if h_mid is not None:
            x = x - self.backout_alpha.to(dtype=x.dtype) * h_mid
