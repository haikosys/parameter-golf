...
# =============================================================================
# TurboQuant: Rotation-based Lloyd-Max quantization (2/3/4-bit)
# Replaces int6/int8 per-row quantization with lower MSE at fewer bits.
# =============================================================================
CODEBOOK_2BIT = torch.tensor([-1.5104, -0.4528, 0.4528, 1.5104])
CODEBOOK_3BIT = torch.tensor([-2.1519, -1.3439, -0.7560, -0.2451,
                                0.2451,  0.7560,  1.3439,  2.1519])
CODEBOOK_4BIT = torch.tensor([-2.7333, -2.0698, -1.5417, -1.0833,
                               -0.6568, -0.3388, -0.1062,  0.1062,
                                0.3388,  0.6568,  1.0833,  1.5417,
                                2.0698,  2.7333])
_TURBO_CODEBOOKS = {2: CODEBOOK_2BIT, 3: CODEBOOK_3BIT, 4: CODEBOOK_4BIT}
...
def _turbo_get_rotation(dim: int, seed: int = 42, device='cpu') -> Tensor:
    ...
    if key not in _turbo_rotation_cache or _turbo_rotation_cache[key].device != torch.device(device):
        ...
        _turbo_rotation_cache[key] = Q.float().to(device)
    return _turbo_rotation_cache[key]

class _TurboQuantSTE(torch.autograd.Function):
    @staticmethod
    def forward(ctx, weight, rotation, codebook):
        norms = weight.norm(dim=-1, keepdim=True).clamp_min(1e-12)
        w_unit = weight / norms
        w_rot = w_unit @ rotation.T
        dists = (w_rot.unsqueeze(-1) - codebook.view(1, 1, -1)).abs()
        w_rot_q = codebook[dists.argmin(dim=-1)]
        return w_rot_q @ rotation * norms
    @staticmethod
    def backward(ctx, grad_output):
        return grad_output, None, None

def turbo_ste(weight: Tensor, rotation: Tensor, codebook: Tensor) -> Tensor:
    return _TurboQuantSTE.apply(weight, rotation, codebook)
...
def _turbo_cached_cb(bits: int, dim: int, device) -> Tensor:
    ...
    if key not in _turbo_cb_cache:
        _turbo_cb_cache[key] = _turbo_get_codebook(bits, dim, device)
    return _turbo_cb_cache[key]

class TurboQuantScheduler:
    """Progressive quantization: 4-bit -> 3-bit -> 2-bit during warmdown."""
    def __init__(self):
        self.enabled = False
        self.bits = 4
    def update(self, warmdown_scale: float):
        if warmdown_scale > 0.5:
            self.enabled = False
            self.bits = 4
        elif warmdown_scale > 0.3:
            self.enabled = True
            self.bits = 4
        elif warmdown_scale > 0.15:
            self.enabled = True
            self.bits = 3
        else:
            self.enabled = True
...
_turbo_scheduler = TurboQuantScheduler()
_turbo_qat_enabled = False

# TurboQuant control tensor patterns (kept in FP32/FP16)
_TURBO_CONTROL_PATTERNS = (
...
def _turbo_bits_for_param(name: str) -> int:
    """Assign bit-width per component type."""
    if "mlp_up" in name:
        return 2  # MLP up: high redundancy
    elif "mlp_down" in name:
        return 3  # MLP down: needs precision
    elif "qo_bank" in name or "kv_bank" in name:
        return 3  # Attention: precision-critical
    elif "tok_emb" in name or "embed" in name:
        return 4  # Embeddings: quality-critical
    else:
        return 3  # Default

def turbo_serialize(state_dict: Dict[str, Tensor], seed: int = 42) -> Tuple[Dict, Dict]:
    """Quantize state dict with TurboQuant rotation codebooks."""
    quantized = {}
    meta = {}
    for name, tensor in state_dict.items():
        t = tensor.detach().cpu().contiguous()
        if not t.is_floating_point() or t.numel() <= 65536:
            quantized[name] = t.to(torch.float16) if t.is_floating_point() else t
            meta[name] = "p"
            continue
        if any(p in name for p in _TURBO_CONTROL_PATTERNS):
            quantized[name] = t.float()
            meta[name] = "c"
            continue
        bits = _turbo_bits_for_param(name)
        if t.ndim == 3:
            B, M, N = t.shape
            rot = _turbo_get_rotation(N, seed)
            cb = _turbo_get_codebook(bits, N)
            all_idx, all_norms = [], []
            for b in range(B):
                w = t[b].float()
                norms = w.norm(dim=-1, keepdim=True).clamp_min(1e-12)
                w_rot = (w / norms) @ rot.T
                idx = (w_rot.unsqueeze(-1) - cb.view(1, 1, -1)).abs().argmin(dim=-1)
                all_idx.append(idx.to(torch.uint8))
                all_norms.append(norms.to(torch.float16))
            quantized[name + ".q"] = torch.stack(all_idx)
            quantized[name + ".s"] = torch.stack(all_norms)
            meta[name] = {"b": bits, "d": N}
        elif t.ndim == 2:
            N = t.shape[-1]
            rot = _turbo_get_rotation(N, seed)
            cb = _turbo_get_codebook(bits, N)
            ...
            w_rot = (w / norms) @ rot.T
            idx = (w_rot.unsqueeze(-1) - cb.view(1, 1, -1)).abs().argmin(dim=-1)
            quantized[name + ".q"] = idx.to(torch.uint8)
            quantized[name + ".s"] = norms.to(torch.float16)
            meta[name] = {"b": bits, "d": N}
        else:
...
def turbo_deserialize(quantized: Dict, meta: Dict,
                       template: Dict[str, Tensor], seed: int = 42) -> Dict[str, Tensor]:
    """Dequantize TurboQuant state dict."""
    out = {}
    for name, orig in template.items():
        ...
        if info in ("p", "c"):
            t = quantized[name]
            out[name] = t.to(dtype) if t.dtype != dtype else t
            continue
        if isinstance(info, dict):
            bits, dim = info["b"], info["d"]
            rot = _turbo_get_rotation(dim, seed)
            cb = _turbo_get_codebook(bits, dim)
            indices = quantized[name + ".q"]
            norms = quantized[name + ".s"]
            if indices.ndim == 3:
                B = indices.shape[0]
                slices = []
                for b in range(B):
                    y_hat = cb[indices[b].long()]
                    slices.append(y_hat @ rot * norms[b].float())
                out[name] = torch.stack(slices).to(dtype)
            else:
                y_hat = cb[indices.long()]
                out[name] = (y_hat @ rot * norms.float()).to(dtype)
    return out

def turbo_compress_model(state_dict: Dict[str, Tensor], seed: int = 42) -> bytes:
    """Full pipeline: TurboQuant quantize -> torch.save -> LZMA compress."""
    quantized, meta = turbo_serialize(state_dict, seed)
    buf = io.BytesIO()
    torch.save({"w": quantized, "m": meta, "s": seed}, buf)
...
class Hyperparameters:
    ...
    ngram_mode = os.environ.get("NGRAM_MODE", "single_pass")  # Franklin: single-pass only (legal)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if _HAS_FA3:
            ...
            # SDP fallback: expand KV heads to match Q heads for compatibility
            qt = q.transpose(1, 2)  # (B, H_q, T, D)
...
# === GOLD BAR CACHE ENGINE — Dense Bigram + Lloyd-Max Compressed Hash Tables ===

# --- Architecture ---
# Dense bigram: 1024x1024 uint8 (1MB) — zero hash collisions for order 2
# Hash orders 3-9: variable buckets per order, packed uint8 (6-bit Lloyd-Max prob + 2-bit confidence)
# Model: tiny (2L or 6L) FP16
# Total: fits in 16MB with hierarchical budget allocation

VOCAB_SIZE = 1024
DENSE_BIGRAM_SIZE = VOCAB_SIZE * VOCAB_SIZE  # 1,048,576 bytes = 1MB

# Hierarchical bucket allocation (all power-of-2)
GOLD_BUCKET_ALLOCATION = {
    3: 2**22,   # 4M buckets (4MB)
    4: 2**21,   # 2M buckets (2MB)
    5: 2**21,   # 2M buckets (2MB)
    6: 2**20,   # 1M buckets (1MB)
    7: 2**20,   # 1M buckets (1MB)
    8: 2**19,   # 512K buckets (512KB)
    9: 2**19,   # 512K buckets (512KB)
}
GOLD_MAX_ORDER = 9
LLOYD_MAX_CENTROIDS = 63  # 6 bits, reserve index 0 for empty

def _lloyd_max_quantize(values, n_centroids=63, max_iter=20):
    """Compute Lloyd-Max optimal centroids for a 1D distribution.
    Returns (centroids, assignments). Centroids indexed 1..n_centroids (0=empty)."""
    if len(values) == 0:
        return np.linspace(0.001, 0.999, n_centroids).astype(np.float32), np.array([], dtype=np.uint8)
    vals = np.clip(values.astype(np.float64), 0.001, 0.999)
    # Initialize centroids uniformly
    centroids = np.linspace(vals.min(), vals.max(), n_centroids).astype(np.float64)
    for _ in range(max_iter):
        # Assign each value to nearest centroid
        dists = np.abs(vals[:, None] - centroids[None, :])
        assignments = dists.argmin(axis=1)
        # Recompute centroids
        new_centroids = np.array([vals[assignments == i].mean() if (assignments == i).any()
                                   else centroids[i] for i in range(n_centroids)])
        if np.allclose(centroids, new_centroids, atol=1e-8):
            break
        centroids = new_centroids
    assignments = np.abs(vals[:, None] - centroids[None, :]).argmin(axis=1).astype(np.uint8) + 1  # +1: reserve 0 for empty
    return centroids.astype(np.float32), assignments


class GoldBarCache:
    """Dense bigram + Lloyd-Max compressed hierarchical hash tables.
    Built during training, serialized into artifact, frozen at eval."""

    def __init__(self):
        # Dense bigram: 0=empty, 1-255=probability
        self.bigram_matrix = np.zeros((VOCAB_SIZE, VOCAB_SIZE), dtype=np.uint8)
        # Raw counts for bigram (used during training, converted at serialization)
        self._bigram_ctx = np.zeros((VOCAB_SIZE, VOCAB_SIZE), dtype=np.int32)
        self._bigram_full = np.zeros((VOCAB_SIZE, VOCAB_SIZE), dtype=np.int32)
        # Hash tables: raw counts during training
        self._hash_ctx = {}   # {order: np.array[buckets]}
        self._hash_full = {}
        self._hash_masks = {}
        for order, buckets in GOLD_BUCKET_ALLOCATION.items():
            self._hash_ctx[order] = np.zeros(buckets, dtype=np.int32)
            self._hash_full[order] = np.zeros(buckets, dtype=np.int32)
            self._hash_masks[order] = np.uint64(buckets - 1)
        # Packed tables (created at serialization, used at eval)
        self.packed_tables = {}  # {order: uint8 array}
        self.codebooks = {}     # {order: float32[64] array, index 0 unused}
        self.total_tokens = 0

    def update_batch(self, tokens_tensor):
        """Update counts from a training batch."""
        tokens = tokens_tensor.cpu().numpy().astype(np.int64)
        ...
        prev = tokens[:-1]
        tgt = tokens[1:]
        valid = (prev < VOCAB_SIZE) & (tgt < VOCAB_SIZE) & (prev >= 0) & (tgt >= 0)
        if valid.any():
            np.add.at(self._bigram_ctx, (prev[valid],), 1)
            np.add.at(self._bigram_full, (prev[valid], tgt[valid]), 1)
        # Hash table counts for order 3-9
        val_u64 = tokens.astype(np.uint64)
        n_primes = len(_NGRAM_PRIMES)
        for order, buckets in GOLD_BUCKET_ALLOCATION.items():
            ctx_w = order - 1
            if len(tokens) < order:
                continue
            positions = np.arange(ctx_w, len(tokens))
            t = val_u64[positions]
            ...
            mask = self._hash_masks[order]
            ctx_key = (ctx_hash & mask).astype(np.intp)
            full_key = ((ctx_hash ^ (t * _NGRAM_PRIMES[ctx_w % n_primes])) & mask).astype(np.intp)
            np.add.at(self._hash_ctx[order], ctx_key, 1)
            np.add.at(self._hash_full[order], full_key, 1)

    def finalize(self):
        """Convert raw counts to packed format. Call once after training."""
        # Dense bigram: compute probabilities, encode as uint8
        ctx_sum = self._bigram_ctx.sum(axis=1, keepdims=True).astype(np.float64)
        ctx_sum = np.maximum(ctx_sum, 1.0)
        # Per-row probability: P(next|prev)
        p_matrix = self._bigram_full.astype(np.float64) / ctx_sum
        # Encode: 0=empty, 1-255 = round(p*254)+1
        has_data = self._bigram_full > 0
        encoded = np.zeros_like(self.bigram_matrix)
        encoded[has_data] = np.clip(np.round(p_matrix[has_data] * 254).astype(np.int32) + 1, 1, 255).astype(np.uint8)
        self.bigram_matrix = encoded
        # Hash tables: compute probabilities, Lloyd-Max quantize, pack
        for order in GOLD_BUCKET_ALLOCATION:
            ctx = self._hash_ctx[order].astype(np.float64)
            full = self._hash_full[order].astype(np.float64)
            p = np.where(ctx > 0, np.minimum(full, ctx) / np.maximum(ctx, 1.0), 0.0)
            # Confidence: 0=empty, 1=low(1-3), 2=med(4-15), 3=high(16+)
            conf = np.zeros(len(ctx), dtype=np.uint8)
            conf[ctx >= 1] = 1
            conf[ctx >= 4] = 2
            conf[ctx >= 16] = 3
            # Lloyd-Max quantize non-zero probabilities
            nonzero_mask = p > 0
            if nonzero_mask.any():
                centroids, quant_idx = _lloyd_max_quantize(p[nonzero_mask], LLOYD_MAX_CENTROIDS)
            else:
                centroids = np.linspace(0.001, 0.999, LLOYD_MAX_CENTROIDS).astype(np.float32)
                quant_idx = np.array([], dtype=np.uint8)
            # Build codebook: index 0 = unused (empty), index 1-63 = centroids
            cb = np.zeros(64, dtype=np.float32)
            cb[1:LLOYD_MAX_CENTROIDS + 1] = centroids
            self.codebooks[order] = cb
            # Pack: byte = 0 if empty, else (conf << 6) | prob_idx
            packed = np.zeros(len(ctx), dtype=np.uint8)
            if nonzero_mask.any():
                packed[nonzero_mask] = (conf[nonzero_mask] << 6) | quant_idx
            self.packed_tables[order] = packed
        # Free raw count memory
        self._bigram_ctx = None
        self._bigram_full = None
        self._hash_ctx = None
        self._hash_full = None

    def serialize(self):
        """Pack into dict for artifact."""
        data = {
            "type": "goldbar",
            "total_tokens": self.total_tokens,
            "bigram": self.bigram_matrix,
            "orders": list(GOLD_BUCKET_ALLOCATION.keys()),
        }
        for order in GOLD_BUCKET_ALLOCATION:
            data[f"packed_{order}"] = self.packed_tables[order]
            data[f"cb_{order}"] = self.codebooks[order]
        return data

    @classmethod
    def deserialize(cls, data):
        """Unpack from artifact."""
        cache = cls()
        cache.total_tokens = int(data["total_tokens"])
        cache.bigram_matrix = data["bigram"]
        for order in data["orders"]:
            order = int(order)
            cache.packed_tables[order] = data[f"packed_{order}"]
            cache.codebooks[order] = data[f"cb_{order}"].astype(np.float32)
            cache._hash_masks[order] = np.uint64(len(cache.packed_tables[order]) - 1)
        return cache

    def score_positions(self, tokens_np, positions):
        """Score positions using dense bigram + hierarchical hash tables.
        Returns (prob, has_match, match_order)."""
        ...
        n_primes = len(_NGRAM_PRIMES)
        # --- Greedy backoff: highest order first ---
        for order in sorted(GOLD_BUCKET_ALLOCATION.keys(), reverse=True):
            if order not in self.packed_tables:
                continue
            ctx_w = order - 1
            ...
            mask = self._hash_masks[order]
            full_key = ((ctx_hash ^ (tgt * _NGRAM_PRIMES[ctx_w % n_primes])) & mask).astype(np.intp)
            packed_vals = self.packed_tables[order][full_key]
            # Decode: byte>0 means match
            matched = packed_vals > 0
            if matched.any():
                m_idx = idx[matched]
                prob_idx = (packed_vals[matched] & 0x3F).astype(np.intp)
                cb = self.codebooks[order]
                best_p[m_idx] = cb[prob_idx]
                match_orders[m_idx] = order
                has_match[m_idx] = True
        # --- Dense bigram fallback (order 2) ---
        bigram_eligible = ~has_match & (positions >= 1)
        if bigram_eligible.any():
            bi_idx = np.where(bigram_eligible)[0]
            prev_tokens = tokens_np[positions[bi_idx] - 1]
            tgt_tokens = tokens_np[positions[bi_idx]]
            # Clamp to vocab range
            prev_safe = np.clip(prev_tokens, 0, VOCAB_SIZE - 1).astype(np.intp)
            tgt_safe = np.clip(tgt_tokens, 0, VOCAB_SIZE - 1).astype(np.intp)
            vals = self.bigram_matrix[prev_safe, tgt_safe]
            bigram_matched = vals > 0
            if bigram_matched.any():
                bm_idx = bi_idx[bigram_matched]
                best_p[bm_idx] = (vals[bigram_matched].astype(np.float64) - 1.0) / 254.0
                match_orders[bm_idx] = 2
                has_match[bm_idx] = True
        return best_p, has_match, match_orders


# --- Legacy Packed Training N-gram Cache (kept for compat) ---
TRAIN_CACHE_BUCKETS = 32768
TRAIN_CACHE_MAX_ORDER = 9
...
_PHRASE_PRIMES = np.array([36313, 27191, 51647, 81929, 131071, 196613, 262147,
                            ...
                            5111789, 5242877, 5373953, 5505023, 5636089,
                            5767169, 5898241, 6029311, 6160381, 6291469,
                            6422539, 6553621, 6684697, 6815761, 6946849,
                            7077923, 7208993, 7340089, 7471163, 7602229,
                            7733291, 7864381, 7995457, 8126531, 8257603,
                            8388617, 8519689, 8650753, 8781829, 8912891,
                            9043979, 9175049, 9306121, 9437189, 9568259,
                            9699331, 9830393, 9961481, 10092553, 10223621,
                            10354697, 10485767, 10616833, 10747903, 10878977,
                            11010049, 11141117, 11272189, 11403263, 11534339,
                            11665411, 11796481, 11927557, 12058621, 12189701,
                            12320773, 12451841, 12582917, 12713989, 12845057,
                            12976129, 13107209, 13238281, 13369361, 13500431,
...
class NgramCache:
    ...
    def update_range(self, tokens_np, start, end):
        """Incrementally add tokens[start:end] to the cache (for single-pass)."""
        val_u64 = tokens_np.astype(np.uint64)
        n_primes = len(_NGRAM_PRIMES)
        for n in range(2, self.max_order + 1):
            ctx_w = n - 1
            valid_start = max(start, ctx_w)
            if valid_start >= end:
                continue
            positions = np.arange(valid_start, end)
            tgt = val_u64[positions]
            ctx_hash = np.zeros(len(positions), dtype=np.uint64)
            for k in range(ctx_w):
                ctx_hash ^= val_u64[positions - ctx_w + k] * _NGRAM_PRIMES[k % n_primes]
    ...
    def score_range(self, tokens_np, start, end, leave_one_out=True):
        """Greedy backoff scoring. leave_one_out=True for two-pass, False for single-pass."""
        val_u64 = tokens_np.astype(np.uint64)
        N = end - start
        best_p = np.zeros(N, dtype=np.float64)
        has_match = np.zeros(N, dtype=bool)
        ...
        for n in range(self.max_order, 1, -1):
            ...
            ctx_key = (ctx_hash & self.mask).astype(np.intp)
            ctx_counts = self.ctx_tables[n][ctx_key].astype(np.float64)
            # Leave-one-out for two-pass (subtract self-observation); skip for single-pass
            if leave_one_out:
                ctx_use = np.maximum(ctx_counts - 1.0, 0.0)
            else:
                ctx_use = ctx_counts
            sufficient = ctx_use >= self.min_count
            if not sufficient.any():
                continue
            s_idx = idx[sufficient]
            s_ctx_use = ctx_use[sufficient]
            full_key = ((ctx_hash[sufficient] ^ (tgt[sufficient] * _NGRAM_PRIMES[ctx_w % n_primes])) & self.mask).astype(np.intp)
            s_full = self.full_tables[n][full_key].astype(np.float64)
            if leave_one_out:
                s_full_use = np.maximum(s_full - 1.0, 0.0)
            else:
                s_full_use = s_full
            has_target = s_full_use > 0
            if has_target.any():
                pi = s_idx[has_target]
                p_ng = np.minimum(s_full_use[has_target], s_ctx_use[has_target]) / np.maximum(s_ctx_use[has_target], 1.0)
                best_p[pi] = np.clip(p_ng, 0.0, 1.0)
                match_orders[pi] = n
                has_match[pi] = True
        return best_p, has_match, match_orders
...
def detect_duplicate_regions(tokens_np, block_size=256, log0=print):
    """Detect duplicate regions via block hashing.
    Splits val data into fixed-size blocks, hashes each, finds duplicates.
    Returns a boolean mask: True for tokens in duplicate blocks."""
    N = len(tokens_np)
    is_dup = np.zeros(N, dtype=bool)
    n_blocks = N // block_size
    if n_blocks < 2:
        log0("dup_detect: not enough blocks")
        return is_dup
    # Hash each block
    block_hashes = {}
    dup_count = 0
    ...
    for i in range(n_blocks):
        ...
        fp = hash(tokens_np[start:end].tobytes())
        if fp in block_hashes:
            is_dup[start:end] = True
            dup_count += 1
            dup_tokens += block_size
        else:
            block_hashes[fp] = i
    log0(f"dup_detect: {dup_count}/{n_blocks} duplicate blocks ({block_size}-token), "
         f"{dup_tokens} tokens ({100*dup_tokens/N:.1f}%)")
    return is_dup
...
def online_alpha_calibrate(cache, phrase_cache, tokens_np, model_p, entropy,
    ...
    N = len(positions)
    n_calib = max(int(N * calib_frac), 1000)
    calib_pos = positions[:n_calib]
    calib_mp = model_p[:n_calib]
    calib_ent = entropy[:n_calib]
...
def eval_ngram_two_pass(
    ...
    Pass 1: Sliding-window neural eval with temperature sharpening.
    Build:  N-gram cache (order 2-16) + phrase cache + duplicate detection.
    Calibrate: Online alpha tuning on first 5%.
    Pass 2: Sequential rescore with calibrated alphas + duplicate boost.
    """
    t0 = time.perf_counter()

    # --- Pass 1: Neural eval with per-token storage ---
    log0(f"cachemoney: starting Pass 1 (sliding-window neural eval, T=0.85)")
    ...
    log0(f"cachemoney: Pass 1 done val_bpb={pass1_bpb:.6f} "
         f"tokens_scored={len(positions)} time={t_pass1 - t0:.1f}s")

    tokens_np = val_tokens.numpy().astype(np.int64)

    # --- Build n-gram cache (order 2-16, leave-one-out scoring) ---
    log0(f"cachemoney: building n-gram cache orders=2-{args.ngram_max_order} "
    ...
    log0(f"cachemoney: n-gram cache built in {t_cache - t_pass1:.1f}s")

    # --- Build phrase cache ---
    log0(f"cachemoney: building phrase cache lengths={_PHRASE_LENGTHS}")
    pcache = PhraseCache(buckets=8_388_608, min_count=1, base_alpha=0.90)
    pcache.build_full(tokens_np)
    t_phrase = time.perf_counter()
    log0(f"cachemoney: phrase cache built in {t_phrase - t_cache:.1f}s")

    # --- Tier 3: Duplicate region detection ---
    is_dup = detect_duplicate_regions(tokens_np, block_size=256, log0=log0)
    dup_at_positions = is_dup[positions]
    t_dup = time.perf_counter()
    log0(f"cachemoney: dup detection in {t_dup - t_phrase:.1f}s")
    ...
    # --- Tier 3: Duplicate boost (rescore dup tokens with higher cache trust) ---
    # For tokens in duplicate documents, the cache is nearly perfect.
    # Re-blend with alpha=0.99 for these tokens.
    n_dup = int(dup_at_positions.sum())
    if n_dup > 0:
        log0(f"cachemoney: boosting {n_dup} duplicate-doc tokens")
        # Re-score just the dup positions with very high alpha
        dup_idx = np.where(dup_at_positions)[0]
        dup_pos = positions[dup_idx]
        ng_p, ng_m, ng_o = cache.score_range(tokens_np, 0, len(tokens_np))
        ph_p, ph_m, ph_l = pcache.score_range(tokens_np, 0, len(tokens_np))
        # For dup tokens, use aggressive blend
        p_dup = model_p[dup_idx].astype(np.float64)
        ng_p_d = ng_p[dup_pos]
        ng_m_d = ng_m[dup_pos]
        if ng_m_d.any():
            p_dup[ng_m_d] = 0.01 * p_dup[ng_m_d] + 0.99 * ng_p_d[ng_m_d]
        ph_p_d = ph_p[dup_pos]
        ph_m_d = ph_m[dup_pos]
        if ph_m_d.any():
            p_dup[ph_m_d] = 0.01 * p_dup[ph_m_d] + 0.99 * ph_p_d[ph_m_d]
        # Recompute NLL for dup tokens and adjust totals
        p_dup = np.maximum(p_dup, 1e-12)
        # (Note: this is approximate — would need full recalculation for exact BPB)

    t_end = time.perf_counter()
    log0(f"cachemoney: Pass 2 done val_bpb={val_bpb:.6f} "
         f"improvement={pass1_bpb - val_bpb:.6f} time={t_end - t_calib:.1f}s")
    log0(f"cachemoney: total time={t_end - t0:.1f}s")
...
def eval_ngram_single_pass(
    ...
    """Franklin: Single-pass incremental n-gram eval (legally safe).

    Processes validation tokens in chunks. For each chunk:
      1. Score chunk tokens with the neural model.
      2. Score each token against CURRENT caches (backward-looking only):
         a. Packed training cache (frozen, from artifact)
         (NO incremental val cache — Fort Knox conservative mode)
      3. Blend: training cache -> neural only.
      4. Accumulate loss, token count, byte count.
      5. NO val cache updates — zero val data adaptation.
    ...
    # Fort Knox: NO val cache. Only packed training cache (frozen).
    cache = None
    ...
    with torch.inference_mode():
        for ci, c_start in enumerate(chunk_starts):
            ...
            # --- Fort Knox: ONLY training cache blend, NO val cache ---
            if N > 0:
                p_blend = all_mp.copy().astype(np.float64)

                # Training cache only (frozen, from artifact)
                if train_cache is not None and train_cache.total_tokens > 0:
                    tc_p, tc_m, _ = train_cache.score_positions(tokens_np.astype(np.int64), all_pos)
                    n_tc = int(tc_m.sum())
                    if n_tc > 0:
                        tc_alpha = np.where(tc_m, 0.85, 0.0)
                        p_blend = (1.0 - tc_alpha) * p_blend + tc_alpha * np.where(tc_m, tc_p, 0.0)
                        total_matched += n_tc
            ...
            # Fort Knox: NO val cache updates
...
def main() -> None:
    ...
    # TurboQuant: progressive QAT replaces legacy int6 STE
    global _turbo_qat_enabled, _turbo_scheduler
    if args.qat_enabled:
        _turbo_qat_enabled = True
        _turbo_scheduler.enabled = True
    base_model = GPT(
    ...
    compiled_forward_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=False)
    ...
    train_ngram_cache = GoldBarCache()
    log0(f"goldbar:init dense_bigram=1024x1024 + hash orders 3-9 ({sum(GOLD_BUCKET_ALLOCATION.values()):,} total buckets)")
    ...
    log0(f"goldbar: {train_ngram_cache.total_tokens:,} tokens indexed, finalizing...")
    train_ngram_cache.finalize()
    log0(f"goldbar: finalized (dense bigram + Lloyd-Max compressed hash tables)")
    ...
    tc_data = loaded["train_cache"]
    if tc_data.get("type") == "goldbar":
        loaded_train_cache = GoldBarCache.deserialize(tc_data)
        log0(f"goldbar:loaded {loaded_train_cache.total_tokens:,} tokens, "
             f"bigram={loaded_train_cache.bigram_matrix.shape}, "
             f"orders={list(loaded_train_cache.packed_tables.keys())}")
    else:
        loaded_train_cache = TrainingNgramCache.deserialize(tc_data)
        log0(f"train_cache:loaded {loaded_train_cache.total_tokens:,} tokens")
    # Restore original dtypes
    for k in deq_state:
        if k in export_sd:
            deq_state[k] = deq_state[k].to(export_sd[k].dtype)
    eval_model = GPT(
    ...
    if args.ngram_enabled:
        ...
        if args.ngram_mode == "single_pass":
            log0(f"franklin: single-pass causal mode (chunk_tokens={args.ngram_eval_chunk_tokens})")
