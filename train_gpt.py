...
# === Lloyd-Max 6-bit Rotation Quantization ===
_lm_rot_cache: Dict[Tuple[int,int], Tensor] = {}
def _lm_rotation(dim, seed=42, device='cpu'):
    key = (dim, seed)
    if key not in _lm_rot_cache or _lm_rot_cache[key].device != torch.device(device):
        p2 = 1
        while p2 < dim: p2 *= 2
        ...
        H = H[:dim, :dim]
        if dim != p2: H, _ = torch.linalg.qr(H)
        ...
        _lm_rot_cache[key] = (Q @ H).float().to(device)
    return _lm_rot_cache[key]
def _lm_fit(data, n_levels, iters=15):
    if n_levels <= 1: return torch.zeros(1)
    s = max(abs(data.min().item()), abs(data.max().item()), 0.1) if data.numel() > 0 else 2.5
    cb = torch.linspace(-s, s, n_levels)
    d = data[torch.randint(0, data.numel(), (min(100000, data.numel()),))].float() if data.numel() > 100000 else data.float()
    for _ in range(iters):
        assign = (d.unsqueeze(-1) - cb.unsqueeze(0)).abs().argmin(-1)
        for j in range(n_levels):
            m = assign == j
            if m.any(): cb[j] = d[m].mean()
    return cb.sort().values
_LM_CTRL = ("attn_scale","mlp_scale","resid_mix","q_gain","skip_weight","smear","ve_","dtg_gate","attn_gate","vr_lambda")
def lm_serialize(sd, seed=42):
    q, meta = {}, {}
    for name, tensor in sd.items():
        t = tensor.detach().cpu().contiguous()
        if not t.is_floating_point() or t.numel() <= 65536:
            q[name] = t.to(torch.float16) if t.is_floating_point() else t; meta[name] = "p"; continue
        if any(p in name for p in _LM_CTRL):
            q[name] = t.float(); meta[name] = "c"; continue
        if t.ndim >= 2:
            w = t.float(); N = w.shape[-1]; rot = _lm_rotation(N, seed)
            norms = w.norm(dim=-1, keepdim=True).clamp_min(1e-12)
            w_rot = (w / norms) @ rot.T
            # Static 6-bit codebook scaled to data range (no k-means, ~10s total vs ~180s)
            spread = w_rot.abs().max().item()
            cb = torch.linspace(-spread, spread, 64)  # uniform 64 levels at data scale
            idx = (w_rot.unsqueeze(-1) - cb.view(1, 1, -1)).abs().argmin(-1)
            q[name+".q"] = idx.to(torch.uint8); q[name+".s"] = norms.to(torch.float16)
            q[name+".cb"] = cb.to(torch.float16); meta[name] = {"b": 6, "d": N}
        else:
            q[name] = t.to(torch.float16); meta[name] = "p"
    return q, meta
def lm_deserialize(q, meta, template, seed=42):
    out = {}
    for name, orig in template.items():
        info = meta.get(name)
        if info is None: continue
        if info in ("p","c"): out[name] = q[name].to(orig.dtype); continue
        if isinstance(info, dict):
            dim = info["d"]; rot = _lm_rotation(dim, seed)
            cb = q[name+".cb"].float(); idx = q[name+".q"]; norms = q[name+".s"]
            out[name] = (cb[idx.long()] @ rot * norms.float()).to(orig.dtype)
    return out
def lm_compress(sd, seed=42):
    q, m = lm_serialize(sd, seed); buf = io.BytesIO()
    torch.save({"w": q, "m": m, "s": seed}, buf)
    try: return zstandard.ZstdCompressor(level=19).compress(buf.getvalue()), "zstd"
    except: return lzma.compress(buf.getvalue(), preset=6), "lzma"
def lm_decompress(blob, template):
    try: raw = zstandard.ZstdDecompressor().decompress(blob)
    except: raw = lzma.decompress(blob)
    data = torch.load(io.BytesIO(raw), map_location="cpu", weights_only=False)
    return lm_deserialize(data["w"], data["m"], template, data["s"])
# === END Lloyd-Max ===
class Hyperparameters:
    ...
    iterations = int(os.environ.get("ITERATIONS", 99999))
...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        scale = (clip_abs / 127.0).clamp_min(1e-7)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                ...
                scale = (row_max / 31.0).clamp_min(1e-7)
...
def main() -> None:
    ...
    code_bytes = len(code.encode("utf-8"))
    # Try Lloyd-Max 6-bit rotation quantization (primary path)
    log0("lloydmax: 6-bit rotation quantization on unbanked weights...")
    lm_blob, lm_comp = lm_compress(unbanked_sd, seed=args.seed)
    lm_total = len(lm_blob) + code_bytes
    log0(f"lloydmax: {len(lm_blob)} bytes ({lm_comp}), total={lm_total}")
    if lm_total <= 16_000_000:
        log0(f"lloydmax: FITS ({lm_total} bytes)")
        quant_blob = lm_blob
        deq_unbanked = lm_decompress(lm_blob, unbanked_sd)
    else:
        log0(f"lloydmax: over budget ({lm_total}), falling back to INT6")
        quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"})
        quant_buf = io.BytesIO()
        torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
        quant_blob = lzma.compress(quant_buf.getvalue(), preset=6)  # lzma compresses INT6 better than zstd
        deq_unbanked = dequantize_mixed_int6(quant_result, quant_meta, unbanked_sd)
    if master_process:
        with open("final_model.int6.ptz", "wb") as f:
            f.write(quant_blob)
        log0(f"artifact: {len(quant_blob)} bytes, total: {len(quant_blob) + code_bytes} bytes")
    if distributed:
        dist.barrier()
    ...
    # --- EGGROLL: post-quantization bin refinement on INT6 weights ---
    # Operates during eval budget. Perturbs int8 quantized values ±1, keeps improvements.
    if master_process:
        log0("eggroll_int6: starting bin refinement on quantized weights...")
    eggroll_budget_s = 60
    t_egg = time.perf_counter()
    # Load quantized state for manipulation
    with open("final_model.int6.ptz", "rb") as f:
        _egg_blob = f.read()
    try:
        _egg_raw = zstandard.ZstdDecompressor().decompress(_egg_blob)
    except Exception:
        _egg_raw = lzma.decompress(_egg_blob)
    _egg_state = torch.load(io.BytesIO(_egg_raw), map_location="cpu")
    _egg_q, _egg_m = _egg_state["w"], _egg_state["m"]
    # Find quantized tensors (int8 .q keys)
    _egg_qnames = [k for k in _egg_q if k.endswith(".q") and _egg_q[k].dtype == torch.int8]
    if _egg_qnames:
        seq_len = args.train_seq_len
        n_tok = min(val_tokens.numel() - 1, seq_len * 32)
        _sx = val_tokens[:n_tok+1].to(device=device, dtype=torch.int64)[:-1].reshape(-1, seq_len)
        _sy = val_tokens[:n_tok+1].to(device=device, dtype=torch.int64)[1:].reshape(-1, seq_len)
        _eb = min(_sx.shape[0], 8)
        def _egg_eval():
            _deq = dequantize_mixed_int6(_egg_q, _egg_m, unbanked_sd)
            _ds = _rebank_state_dict(_deq, args.num_layers, sd_cpu)
            eval_model.load_state_dict(_ds, strict=True)
            with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                return eval_model(_sx[:_eb], _sy[:_eb]).item()
        _bl = _egg_eval()
        _step, _impr = 0, 0
        log0(f"eggroll_int6: baseline={_bl:.6f} tensors={len(_egg_qnames)} budget={eggroll_budget_s}s")
        while time.perf_counter() - t_egg < eggroll_budget_s:
            _step += 1
            _qn = _egg_qnames[_step % len(_egg_qnames)]
            _qt = _egg_q[_qn]; _flat = _qt.reshape(-1).to(torch.int16)
            _nw = _flat.numel(); _np = min(_nw, 1024)
            _mi = torch.randint(0, _nw, (_np,)); _saved = _flat[_mi].clone()
            _flat[_mi] = (_saved + 1).clamp(-32, 31)
            _egg_q[_qn] = _flat.to(torch.int8).reshape(_qt.shape)
            _lp = _egg_eval()
            _flat[_mi] = (_saved - 1).clamp(-32, 31)
            _egg_q[_qn] = _flat.to(torch.int8).reshape(_qt.shape)
            _lm = _egg_eval()
            if _lp < _lm and _lp < _bl:
                _flat[_mi] = (_saved + 1).clamp(-32, 31)
                _egg_q[_qn] = _flat.to(torch.int8).reshape(_qt.shape)
                _bl = _lp; _impr += 1
            elif _lm < _lp and _lm < _bl:
                _bl = _lm; _impr += 1
            else:
                _flat[_mi] = _saved
                _egg_q[_qn] = _flat.to(torch.int8).reshape(_qt.shape)
            if _step % 5 == 0:
                log0(f"eggroll_int6: step={_step} loss={_bl:.6f} impr={_impr} t={time.perf_counter()-t_egg:.1f}s")
        log0(f"eggroll_int6: done steps={_step} impr={_impr} loss={_bl:.6f} t={time.perf_counter()-t_egg:.1f}s")
        # Reload improved weights
        _deq_final = dequantize_mixed_int6(_egg_q, _egg_m, unbanked_sd)
        deq_state = _rebank_state_dict(_deq_final, args.num_layers, sd_cpu)
        eval_model.load_state_dict(deq_state, strict=True)
    else:
        log0("eggroll_int6: no quantized tensors found, skipping")
    log0(f"eggroll_int6: total time={time.perf_counter()-t_egg:.1f}s")
    # --- END EGGROLL ---
